...
fs.readFile("/dev/AOC_22/day07/p2/input.data", 'utf8', (err, data) => {
    if (err) {
        console.error(err);
        return;
    }
    ...
    console.log(`Total occupied space is ${rootFolder.getSize()}`);
    const totalDiskSpace = 70000000;
    const neededDiskSpace = 30000000 - (totalDiskSpace - rootFolder.getSize());
    flatFolders.sort((a,b) => a.getSize() - b.getSize());
    for (let folder of flatFolders) {
        if (folder.getSize() > neededDiskSpace) {
            console.log(`Found the directory which will free up ${folder.getSize()}`);
            break;
        }
    }
